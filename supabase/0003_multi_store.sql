-- =============================================================
-- 0003_multi_store — 複数店舗対応
--   - stores / store_members / shift_periods.store_id
--   - オーナーは全店舗を管理、従業員は所属店舗の期間のみ閲覧・提出
--   - 既存データは「本店」を作成して移行
-- 前提: 0001・0002 適用後。冪等。
-- =============================================================
...
-- 3) 提出期間に店舗を追加（nullable で追加 → 移行 → not null）
...
-- shift_preferences: 「受付中」に加えて「所属店舗の期間」を要求
...
-- ----- 適用記録 ----------------------------------------------
insert into schema_migrations (version) values ('0003_multi_store')
