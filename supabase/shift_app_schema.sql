-- =============================================================
-- 飲食店向けシフト管理アプリ — Supabase (PostgreSQL) スキーマ v2
-- 運用: 従業員が「希望シフト」を提出 → オーナーが確定シフトを作成
-- 前提: 従業員の大半は非正規（アルバイト・パート）
...
create type preference_type  as enum ('preferred', 'available', 'unavailable'); -- 希望 / 勤務可能 / NG(休み希望)
...
create table profiles (
  ...
  employment_type     employment_type not null default 'part_time', -- 大半が非正規なので既定値
  max_hours_per_month numeric,                  -- 扶養・契約上の月間上限の目安(任意)。シフト調整時の参考に
  phone               text,
  is_active           boolean         not null default true,  -- 退職者は false に（削除せず無効化）
  created_at          timestamptz     not null default now()
);

-- =============================================================
-- 2. shift_periods : 希望提出の単位（例「2026年6月後半」＋締切）
--    オーナーが作成し、提出を受け付ける期間を管理
-- =============================================================
create table shift_periods (
  id                  uuid          primary key default gen_random_uuid(),
  title               text          not null,    -- 例: '2026年6月後半'
  start_date          date          not null,
  end_date            date          not null,
  submission_deadline timestamptz   not null,    -- 希望提出の締切
...
-- =============================================================
-- 3. shift_preferences : 従業員が提出する希望シフト（主たる入力）
--    1日に複数の時間帯を出せるよう、unique制約は設けない
-- =============================================================
create table shift_preferences (
  ...
  start_time  time,                              -- NG(休み希望)の場合は null 可
  end_time    time,
  preference  preference_type not null default 'preferred',
  note        text,
  created_at  timestamptz     not null default now()
);
create index idx_pref_period_emp on shift_preferences (period_id, employee_id);
create index idx_pref_date       on shift_preferences (work_date);

-- =============================================================
-- 4. confirmed_shifts : オーナーが希望をもとに確定した実シフト
-- =============================================================
create table confirmed_shifts (
  ...
  position    text,                              -- 例: ホール / キッチン
  note        text,
  created_at  timestamptz not null default now()
);
create index idx_conf_period_emp on confirmed_shifts (period_id, employee_id);
create index idx_conf_date       on confirmed_shifts (work_date);

-- =============================================================
-- 集計ビュー : 確定シフトの合計時間（上限管理の参考に）
--   深夜跨ぎ(end < start)は +24h して計算
--   security_invoker により、各ユーザーのRLSが適用される
-- =============================================================
create view v_confirmed_hours
...
--   最初の登録者をオーナーにする場合は登録後に:
--   UPDATE profiles SET role='owner' WHERE id='対象のuuid';
-- =============================================================
